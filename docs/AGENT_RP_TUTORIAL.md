...
如果你已有 SillyTavern 前端，可以将其后端指向网关，复用 ST 的成熟 UI（立绘、背景、macro、正则、QuickReply）：

1. 打开 SillyTavern 前端
2. 设置 → API → 将后端地址改为 `http://localhost:3000`
3. 填入网关 Token

网关提供 ST 兼容路由 shim（`/api/characters`、`/api/chats`、`/api/presets`、`/api/worldinfo`、`/api/generate`），在 Agent 模式下 `/api/generate` 会触发 Agent 引擎而非传统单次调用。

### 3.3 Agent 专用前端（Agent 剧场）

打开浏览器访问网关面板 `http://localhost:3000/panel.html`，切换到"Agent 剧场"页面：
...
│   状态面板    │     正文流（实时）    │      时间线           │
│              │                      │                       │
│  时间：傍晚   │  [Agent 正在生成...] │  · state.read         │
│  地点：实验室 │                      │  · memory.recall      │
│  在场：2人    │                      │  · narrative.generate │
│              │                      │  · state.write        │
│              │                      │                       │
│              │ [选项1] [选项2]       │                       │
│              │ [选项3] [选项4]       │                       │
...
特性：
- **实时流式输出**：通过 SSE 订阅，不刷新页面
- **状态面板**：显示当前时间 / 地点 / 在场角色 / 各角色状态
- **时间线**：从事件流重建，显示每次工具调用 / 子代理审查 / 状态变更
- **边玩边改**：右侧配置侧栏可编辑 YAML Profile，保存后热重载，会话不中断
